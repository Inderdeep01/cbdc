...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    ...
    HOST="peer0.hdfc.bank.cbdc"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.axis.bank.cbdc"
    PORT=10051
  else
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.cbdc -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
